...
describe("estimateDraftComplexity", () => {
      ...
      rooms: [
        {
          ...
          scope: "1200 sqft LVP flooring installation",
        },
      ],
    };
  ...
  test("classifies multi-part flooring job as room_remodel for scope confirmation", () => {
    ...
    expect(
      buildScopeChecklist(draft, "room_remodel", flooringNotes).templateKey,
    ).toBe("flooring");
  });
      ...
      rooms: [
        {
          ...
          scope: "walls, ceilings, doors, and trim",
        },
      ],
    };
  ...
  test("keeps interior repaint with localized drywall repair on the painting checklist", () => {
    ...
    const draft = {
      ...
      originalNotes: notes,
      rooms: [
        { name: "Interior Repaint and Bedroom Drywall Repair", scope: notes },
      ],
    };
  ...
  test("applying flooring checklist keeps combined notes material/labor total intact", () => {
    ...
    const checklist = buildScopeChecklist(draft, "room_remodel", notes);
    const confirmed = checklist.items.filter((item) =>
      ["floor_demo", "flooring", "trim"].includes(item.id),
    );
    const next = applyScopeAssumptions(
      { ...draft, scopeChecklist: checklist },
      confirmed,
      checklist.suggestedMeasurements,
    );
  ...
  test("classifies bathroom remodel as room_remodel", () => {
    const notes =
      "Full bathroom remodel, new shower tile, move toilet, quartz vanity";
    const draft = {
      projectType: "bathroom",
    ...
    const cases = [
      {
        ...
        notes:
          "Add a 240 sqft bedroom addition with foundation, framing, electrical, insulation, drywall, and paint.",
      },
      {
        projectType: "home_addition",
        notes:
          "Home addition with new family room, bathroom tie-in, HVAC extension, roofing tie-in, and finishes.",
      },
      {
        projectType: "adu",
        notes:
          "Build a detached ADU casita with kitchenette, bathroom, utility trenching, foundation, framing, and roofing.",
      },
      {
        projectType: "garage_conversion",
        notes:
          "Garage conversion into living space with insulation, drywall, electrical, HVAC, and flooring.",
      },
    ];
  ...
  test("classifies notes-only addition language as addition tier", () => {
    const notes =
      "Room addition: add 320 sqft office with slab foundation, framing, roof tie-in, electrical, drywall and paint.";
  ...
  test("classifies notes-only ground-up language as ground_up tier", () => {
    const notes =
      "Ground up build for a 1850 sqft custom home with sitework, foundation, framing, roofing, MEP, drywall and finishes.";
    const draft = { projectType: "other", rooms: [] };
    expect(classifyEstimateTier(draft, notes)).toBe("ground_up");
  ...
  test("classifies Step 1 plan-import handoff notes as ground_up (not room_remodel)", () => {
    ...
    expect(checklist.items.find((i) => i.id === "framing")?.label).toBe(
      "Framing",
    );
  });
  ...
  test("defaults ground-up plans and permits to Yes even when notes omit permit language", () => {
    const notes =
      "New custom home with architectural plans, foundation, framing, and finishes.";
    const checklist = buildScopeChecklist(
    ...
    expect(
      checklist.items.find((i) => i.id === "plans_engineering")?.state,
    ).toBe("included");
    expect(checklist.items.find((i) => i.id === "permits")?.state).toBe(
      "included",
    );
  });
  ...
  test("keeps ground-up permits excluded when notes say owner pulls permits", () => {
    const notes =
      "New home build. Owner pulls permits. Architectural plans included.";
    const checklist = buildScopeChecklist(
      { projectType: "other", rooms: [] },
      "ground_up",
      notes,
  ...
  test("builds bathroom checklist for remodel", () => {
    ...
    expect(checklist.items.some((i) => i.id === "shower_floor_demo")).toBe(
      true,
    );
    expect(checklist.items.some((i) => i.id === "wet_area_install")).toBe(true);
    expect(checklist.items.some((i) => i.id === "shower_floor_tile")).toBe(
      true,
    );
    expect(checklist.items.some((i) => i.id === "exhaust_fan")).toBe(true);
    expect(
      checklist.items.some((i) => i.id === "adhesive_mastic_removal"),
    ).toBe(false);
    expect(checklist.items.find((i) => i.id === "permits")?.state).toBe(
      "unsure",
    );
  ...
  test("bathroom remodel includes permits only when notes explicitly include them", () => {
    const draft = { projectType: "bathroom", rooms: [] };
    const checklist = buildScopeChecklist(
      draft,
      "room_remodel",
      "Bathroom remodel with permits included in the bid.",
    );
    expect(checklist.items.find((i) => i.id === "permits")?.state).toBe(
      "included",
    );
  ...
  test("plumbing trim in bathroom notes does not auto-include trim & baseboard scope", () => {
    ...
    const checklist = buildScopeChecklist(
      { projectType: "bathroom", rooms: [] },
      "room_remodel",
      notes,
    );
    expect(checklist.items.find((i) => i.id === "plumbing_trim")?.state).toBe(
      "included",
    );
  ...
  test("baseboard language still includes trim scope", () => {
    ...
    const checklist = buildScopeChecklist(
      { projectType: "flooring", rooms: [] },
      "room_remodel",
      notes,
    );
    expect(checklist.items.find((i) => i.id === "trim")?.state).toBe(
      "included",
    );
  });
  ...
  test("builds landscaping checklist from project type", () => {
    ...
    const checklist = buildScopeChecklist(
      draft,
      "room_remodel",
      "Backyard landscaping with sod and pavers",
    );
  ...
  test("applyScopeAssumptions merges inclusions and exclusions", () => {
    ...
    expect(next.inclusions.some((l) => l.includes("Toilet: Replacing"))).toBe(
      true,
    );
  ...
  test("applyScopeAssumptions turns confirmed addition phases into missing-price review packages", () => {
    ...
    expect(next.rooms.every((room) => room.status === "missing_price")).toBe(
      true,
    );
    expect(next.rooms.every((room) => room.priceSource === "missing")).toBe(
      true,
    );
    expect(next.rooms.every((room) => room.applyEligible === false)).toBe(true);
    expect(
      next.rooms.find((room) => room.name === "Plans / engineering")
        .missingPriceItems,
    ).toEqual(
  ...
  test("ADU casita notes preselect addition phases and create phase-based review packages", () => {
    ...
    expect(
      checklist.items
        .filter((item) => item.state === "included")
        .map((item) => item.id),
    ).toEqual(expect.arrayContaining(["excavation", "concrete"]));
    expect(
      checklist.items
        .filter((item) => item.state === "included")
        .some((item) => String(item.id).startsWith("electrical")),
    ).toBe(true);
    ...
    expect(
      packageNames.some((name) =>
        /bathroom demo|kitchen remodel|lvp flooring installation|plumbing \(bathroom\)|interior painting/i.test(
          name,
        ),
      ),
    ).toBe(false);
  ...
  test("garage conversion notes include interior conversion phases without foundation or roof work", () => {
    ...
    const includedIds = checklist.items
      .filter((item) => item.state === "included")
      .map((item) => item.id);
    const excludedIds = checklist.items
    ...
    expect(
      packageNames.some((name) =>
        /bathroom|kitchen|lvp flooring installation/i.test(name),
      ),
    ).toBe(false);
    expect(next.rooms.every((room) => room.status === "missing_price")).toBe(
      true,
  ...
  test("basement finish stays room_remodel and creates generic multi-trade phase packages", () => {
    ...
    expect(
      packageNames.some((name) =>
        /bathroom demo|kitchen remodel|lvp flooring installation/i.test(name),
      ),
    ).toBe(false);
    expect(next.rooms.every((room) => room.status === "missing_price")).toBe(
      true,
  ...
  test("applyScopeAssumptions keeps ground-up build as phase-based missing-price packages", () => {
    ...
    expect(packageNames).not.toEqual(
      expect.arrayContaining(["Builder overhead & profit"]),
    );
    expect(
      packageNames.some((name) =>
        /bathroom|kitchen|lvp flooring installation/i.test(name),
  ...
  test("applyScopeMeasurements stamps sqft and lf on matching scope packages", () => {
      ...
      scopePackages: [
        {
          ...
          scope: "Bathroom floor tile",
          scopeQuantities: [],
        },
        {
          name: "Shower Tile Installation",
          scope: "New shower wall tile",
          scopeQuantities: [],
        },
        {
          name: "Baseboard Installation",
          scope: "New baseboard trim",
          scopeQuantities: [],
        },
        {
          name: "Toilet Installation",
          scope: "Replace toilet",
          scopeQuantities: [],
        },
        {
    ...
    const next = applyScopeMeasurements(draft, {
      sqft: 90,
      lf: 24,
      showerWallTileSqft: 88,
    });
    expect(next.scopeMeasurements.bathroomFloorSqft).toBe(90);
    expect(next.scopeMeasurements.baseboardLf).toBe(24);
    expect(next.scopePackages[0].scopeQuantities[0]).toMatchObject({
      quantity: 90,
      unit: "sqft",
    });
    expect(next.scopePackages[1].scopeQuantities[0]).toMatchObject({
      quantity: 88,
      unit: "sqft",
    });
    expect(next.scopePackages[2].scopeQuantities[0]).toMatchObject({
      quantity: 24,
      unit: "lf",
    });
    expect(next.scopePackages[3].scopeQuantities[0]).toMatchObject({
      quantity: 1,
      unit: "each",
    });
  ...
  test("applyScopeMeasurements preserves pricingAcceptance and stamps Applied M/L without takeoff qty", () => {
    ...
    expect(
      next.scopeMeasurements.pricingAcceptance.interior_trim.totalAmount,
    ).toBe(10118);
  ...
  test("enrichment preserves selected flooring pricing over note-parsed rates", () => {
    ...
    expect(
      next.scopeMeasurements.itemQuantities.flooring__allowance,
    ).toMatchObject({
      quantity: 5950,
      quantitySource: "user_entered",
    });
    expect(
      next.scopeMeasurements.itemQuantities.flooring__material,
    ).toMatchObject({
      quantity: 2550,
      quantitySource: "user_entered",
    });
  ...
  test("applyScopeAssumptions adds Bathroom Demo when demo is confirmed in checklist", () => {
    const draft = {
      ...
      estimateTier: "room_remodel",
      originalNotes:
        "Full bathroom remodel, new shower tile, vanity, toilet, paint",
    ...
    const demo = (next.rooms || []).find((r) =>
      /bathroom demo/i.test(r.name || ""),
    );
    expect(demo).toBeDefined();
    expect(demo.scopeQuantities?.[0]).toMatchObject({
      quantity: 90,
      unit: "sqft",
    });
  });
  ...
  test("applyScopeAssumptions does not duplicate existing note-backed railing package", () => {
    const draft = {
      ...
      rooms: [
        {
          name: "Custom Shower Tile",
          scope: "120 sqft shower tile",
          price: 2400,
          packageStatus: "calculated",
        },
        {
          name: "Metal Railing",
          scope: "48 linear feet metal railing",
          price: 85,
          packageStatus: "partial_pricing",
        },
        {
          name: "Rock Supply",
          scope: "12 tons of rock",
          price: 95,
          packageStatus: "partial_pricing",
        },
      ],
    ...
    expect(
      (next.rooms || []).filter((r) => /railing/i.test(r.name || "")),
    ).toHaveLength(1);
  ...
  test("applyScopeAssumptions syncs flooring checklist pricing without duplicate demo package", () => {
    ...
    expect(next.rooms.find((r) => r.name === "Tile Demo")).toMatchObject({
      price: 2550,
    });
    expect(
      next.rooms.find((r) => r.name === "LVP Flooring Installation"),
    ).toMatchObject({ price: 6587.5 });
    expect(
      next.rooms.find((r) => r.name === "Baseboard Installation"),
    ).toMatchObject({ price: 1540 });
  });
  ...
  test("applyScopeAssumptions adds priced custom scope item to review packages", () => {
    ...
    expect(custom.scopeQuantities?.[0]).toMatchObject({
      quantity: 500,
      unit: "sqft",
    });
    ...
    const synced = syncRoomsFromScopePackages(next, [pkg]).find(
      (r) => r.name === "Demo",
    );
  ...
  test("applyScopeAssumptions adds tub install package with labor and material hints", () => {
    ...
    const next = applyScopeAssumptions(draft, items);
    const tub = (next.rooms || []).find((r) =>
      /tub installation/i.test(r.name || ""),
    );
    ...
    expect(tub.scopeQuantities?.[0]).toMatchObject({
      quantity: 1,
      unit: "each",
    });
  });
  ...
  test("applyScopeAssumptions adds prefab pan package with labor and material hints", () => {
    ...
    const next = applyScopeAssumptions(draft, items);
    const pan = (next.rooms || []).find((r) =>
      /prefab shower pan/i.test(r.name || ""),
    );
  ...
  test("applyScopeMeasurements stamps painting confirmed measurements onto checklist packages", () => {
    const draft = {
      ...
      scopePackages: [
        ...
        },
        {
          name: "Cabinets",
          scope:
            "Includes cabinet boxes, doors, drawer fronts, and face frames.",
    ...
    expect(next.scopePackages[4].scopeQuantities[0]).toMatchObject({
      quantity: 2000,
      unit: "sqft",
    });
  });
