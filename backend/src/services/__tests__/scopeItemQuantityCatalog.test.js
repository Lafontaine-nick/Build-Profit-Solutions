...
describe("scopeItemQuantityCatalog", () => {
  test("floor tile uses bathroom floor sqft, not global stamp on fixtures", () => {
    ...
    const electricalCount = resolveQuantityForChecklistItem(
      "electrical_rough",
      {
    ...
    expect(electricalCount.dualCount).toMatchObject({
      quantity: 2,
      unit: "each",
    });
    ...
    const electricalAllowance = resolveQuantityForChecklistItem(
      "electrical_rough",
      {
    ...
    expect(electricalAllowance.dualAllowance).toMatchObject({
      quantity: 1500,
      unit: "lump_sum",
    });
  });
  ...
  test("shower tile requires shower wall sqft not bathroom floor", () => {
    ...
    const withShower = normalizeScopeMeasurements({
      sqft: 90,
      showerWallTileSqft: 85,
    });
    const showerReady = resolveQuantityForChecklistItem("shower_tile", {
      measurements: withShower,
    });
  ...
  test("room remodel demo uses the drywall demolition area", () => {
    const resolved = resolveQuantityForChecklistItem("demo", {
      measurements: normalizeScopeMeasurements({ wallDemoSqft: 150 }),
    ...
    expect(resolved.rule.quantityHelper).toMatch(/drywall demolition/i);
  });
  ...
  test("stampPackageWithCatalogRules applies lf to baseboard only", () => {
    const pkg = {
      ...
      scope: "Install baseboard",
      scopeQuantities: [],
    };
    const ctx = {
      measurements: normalizeScopeMeasurements({ sqft: 90, lf: 24 }),
    };
    const next = stampPackageWithCatalogRules(pkg, ctx);
  ...
  test("resolveQuantityForPackage marks cleanup as needing pricing when no amount entered", () => {
    const ctx = { measurements: normalizeScopeMeasurements({ sqft: 90 }) };
    const q = resolveQuantityForPackage(
      "Jobsite Cleanup & Disposal",
      "Final clean",
      ctx,
    );
  ...
  test("mixed exterior companion scopes use their dedicated confirmation measurements", () => {
    const measurements = normalizeScopeMeasurements({
      landscapeSqft: 900,
      sidingRepairSqft: 240,
      retainingWallLf: 36,
    });

    expect(
      resolveQuantityForChecklistItem("landscaping", { measurements }),
    ).toMatchObject({ quantity: 900, unit: "sqft", pricingReady: true });
    expect(
      resolveQuantityForChecklistItem("siding_repairs", { measurements }),
    ).toMatchObject({ quantity: 240, unit: "sqft", pricingReady: true });
    expect(
      resolveQuantityForChecklistItem("retaining_wall", { measurements }),
    ).toMatchObject({ quantity: 36, unit: "lf", pricingReady: true });
  });
  ...
  test("ground_up shell items use living floorAreaSqft from plan takeoff", () => {
    ...
    expect(
      String(drywall.missingMessage || drywall.rule?.quantityHelper || ""),
    ).not.toMatch(/repair/i);
  });
  ...
  test("shower floor tile uses shower floor sqft not bathroom floor", () => {
    ...
    const floor = resolveQuantityForChecklistItem("shower_floor_tile", {
      measurements,
    });
    expect(floor.quantity).toBe(25);
    expect(floor.pricingReady).toBe(true);

    const prefabPan = resolveQuantityForChecklistItem("prefab_shower_pan", {
      measurements,
    });
    expect(prefabPan.quantity).toBe(1);
    expect(prefabPan.unit).toBe("each");

    const tilePan = resolveQuantityForChecklistItem("shower_pan", {
      measurements,
    });
    ...
    const tubDemo = resolveQuantityForChecklistItem("tub_demo", {
      measurements,
    });
    expect(tubDemo.quantity).toBe(1);

    const tubInstall = resolveQuantityForChecklistItem("tub_install", {
      measurements,
    });
    expect(tubInstall.quantity).toBe(1);
    expect(tubInstall.unit).toBe("each");

    const prefabInstall = resolveQuantityForChecklistItem("prefab_shower_pan", {
      measurements,
    });
    expect(prefabInstall.quantity).toBe(1);

    const showerFloorDemo = resolveQuantityForChecklistItem(
      "shower_floor_demo",
      {
        measurements,
      },
  ...
  test("painting Walls package uses checklistItemId instead of kitchen cabinets name match", () => {
    ...
    expect(walls.scopeQuantities[0]).toMatchObject({
      quantity: 1500,
      unit: "sqft",
    });
    ...
    expect(cabinets.scopeQuantities[0]).toMatchObject({
      quantity: 25,
      unit: "lf",
    });
  });
