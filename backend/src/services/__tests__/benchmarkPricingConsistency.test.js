...
} = require("../pricingEngine/pricingRangeCatalog");
...
describe("benchmark pricing data consistency (Phase 1 guard)", () => {
  ...
  test("Roofing subcontractor baseline is the approved $575 architectural-shingle rate", () => {
    expect(NATIONAL_TRADE_AVERAGES.roofing).toMatchObject({
      unit: "square",
      material: 250,
      labor: 325,
    });
    expect(PRICING_RANGE_CATALOG.roofing.material.typical).toBe(250);
    expect(PRICING_RANGE_CATALOG.roofing.labor.typical).toBe(325);
  });
});
