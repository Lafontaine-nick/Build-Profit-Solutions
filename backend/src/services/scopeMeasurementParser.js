...
const SQUARES_RE =
  /(\d[\d,]*(?:\.\d+)?)[\s-]*(?:roofing\s+)?squares?\b/gi;
const ROOF_PITCH_RE =
...
function parseMixedPaintingIntents(text) {
  ...
  const windowCount =
    ...
    firstQty(source.match(/\b(\d[\d,]*)\s+(?:replacement\s+)?windows?\b/i)?.[0] || '', /\d[\d,]*/);
  const baseboardLf = firstQty(source.match(/\b(\d[\d,]*)\s*(?:lf|linear\s+feet)\b[^.\n]{0,30}\bbaseboards?\b/i)?.[0] || '', LF_RE);
  const baseboardInstall = has(/\b(?:install|replace|add|new)\b[^.;\n]{0,45}\b(?:baseboards?|trim|molding|moulding)\b/i);
  const doorInstall = has(/\b(?:install|replace|hang|set)\b[^.;\n]{0,45}\bdoors?\b/i);
  const windowInstall = has(/\b(?:install|replace|set)\b[^.;\n]{0,45}\bwindows?\b/i);
  const exteriorTrimPaint = has(
    /\b(?:exterior|outside)\s+trim\b[^.;\n]{0,35}\b(?:paint|painting|finish)\b|\b(?:paint|painting|finish)\b[^.;\n]{0,35}\b(?:exterior|outside)\s+trim\b/i
  );
  ...
  if (baseboardInstall) out.baseboardInstallIntent = true;
  if (baseboardPaint) out.baseboardPaintIntent = true;
  if (doorInstall && doorCount) out.interiorDoorInstallIntent = true;
  if (doorPaint && doorCount) out.interiorDoorPaintIntent = true;
  if (windowInstall && windowCount) out.windowInstallIntent = true;
  if ((casingPaint || windowTrimPaint) && (windowCount || doorCount)) {
    out.casingPaintIntent = true;
  }
  if (baseboardLf && baseboardInstall) {
    out.itemQuantities = {
      baseboard_install: { quantity: baseboardLf, unit: "lf", quantitySource: "notes" },
    };
  }
  if (doorInstall && doorCount) {
    out.itemQuantities = {
      ...(out.itemQuantities || {}),
  ...
  if (windowInstall && windowCount) {
    out.itemQuantities = {
      ...(out.itemQuantities || {}),
      window_install: { quantity: windowCount, unit: "each", quantitySource: "notes" },
    };
  }
  if (doorCount) out.interiorDoorCount = doorCount;
...
function parseScopeMeasurementsFromNotes(notes, ctx = {}) {
  ...
  const combinedPaintLanguage =
    /\bwalls?\s*(?:and|&)\s*(?:the\s+)?ceilings?\b|\bceilings?\s*(?:and|&)\s*(?:the\s+)?walls?\b/i.test(
      blob,
    );
  const paintSqft = (() => {
  ...
  if (
    ...
    (templateKey === "painting" || templateKey === "room_remodel") &&
    paintSqft &&
    (combinedPaintLanguage ||
  ...
  const floorDemoSqft = pickSqftFromClauses([
    /\b(?:floor|flooring|lvp|laminate|vinyl|carpet|tile)\b[^.;\n]{0,60}\b(?:demo|demolition|remove|removal|tear[\s-]?out)\b/,
    /\b(?:demo|demolition|remove|removal|tear[\s-]?out)\b[^.;\n]{0,60}\b(?:floor|flooring|lvp|laminate|vinyl|carpet|tile)\b/,
  ]);
  ...
  const wallDemoSqft = (() => {
    const quantity =
      String.raw`(\d[\d,]*(?:\.\d+)?)\s*(?:sq\.?\s*ft|sqft|sf|square\s+(?:foot|feet))`;
    const surface = String.raw`(?:walls?|soffit|bulkhead|drywall|sheetrock|gypsum)`;
    const demoVerb = String.raw`(?:demo|demolition|demolish|remove|removal|tear[\s-]?out)`;
    const patterns = [
      new RegExp(
        String.raw`\b${demoVerb}\b[^.;,\n]{0,30}?${quantity}\b[^.;,\n]{0,20}\b(?:of\s+)?${surface}\b`,
        "i",
      ),
      new RegExp(
        String.raw`${quantity}\b[^.;,\n]{0,20}\b(?:of\s+)?${surface}\b[^.;,\n]{0,30}?\b${demoVerb}\b`,
        "i",
      ),
      new RegExp(
        String.raw`\b${demoVerb}\b[^.;,\n]{0,45}\b${surface}\b[^.;,\n]{0,15}${quantity}\b`,
  ...
  const roofDeckingRepairNote =
    /\b(?:roof|roofing|shingles?)\b/i.test(blob) &&
  ...
  if (deckSqft && !roofDeckingRepairNote) out.deckSqft = deckSqft;
  ...
  if (
    ...
    if (!out.roofSquares) {
      ...
      if (sq) out.roofSquares = sq;
        ...
        // Only use an area from the same roof/shingle clause. A nearby
        // "180 sqft decking" repair must not become 1.8 roofing squares.
        const roofClause = String(text)
          .split(/[.;,\n]+/)
          .find((clause) => /\broof|\bshingle/i.test(clause));
        const sqft = roofClause ? firstQty(roofClause, SQFT_RE) : null;
        if (sqft) {
          out.roofAreaSqft = sqft;
          out.roofSquares = Math.round((sqft / 100) * 10) / 10;
        }
  ...
  for (const clause of clauses) {
    if (
      !/\bexcavat(?:e|ion)\b|\btrench(?:ing)?\b|\bgrading\b/.test(
        clause.toLowerCase(),
      )
    )
      continue;
  ...
  if (
    !excavationCy &&
    /\bexcavat(?:e|ion)\b|\btrench(?:ing)?\b|\bgrading\b/.test(blob)
  ) {
  ...
  const mixedPainting = parseMixedPaintingIntents(text);
  const itemQuantities = {
    ...itemAllowances,
    ...itemRatePricing,
    ...(mixedPainting.itemQuantities || {}),
  };
  for (const [key, value] of Object.entries(mixedPainting)) {
    if (key !== "itemQuantities") out[key] = value;
  }
