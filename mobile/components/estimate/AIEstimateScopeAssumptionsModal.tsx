...
import {
  BATHROOM_PAINT_REPAIR_SEVERITY_OPTIONS,
  bathroomPaintRepairUsesAutoFlow,
  formatPaintRepairAutoFlowSummary,
  paintRepairScopeSelectionComplete as isPaintRepairScopeReady,
...
import {
  KITCHEN_BACKSPLASH_DEMO_DIFFICULTY_OPTIONS,
  resolveKitchenBacksplashDemoDifficulty,
  type KitchenBacksplashDemoDifficulty,
...
import {
  ...
  type QmBathroomFixturesPanelsHandle,
  ...
  qmExistingScopePanelStyle,
  qmInstallScopePanelStyle,
...
/** Apply/confirm presses are already scheduled by ReliableFlowPress. Keep the
 * visible state handoff in the same press cycle instead of queueing a second
 * RAF + InteractionManager pass, which made taps appear to be ignored. */
...
function inputShellStyle(
  ...
  return qmInactiveMeasurementShellStyle(darkMode, Colors, options);
}
...
function ScopeIntelligenceNotice({
  ...
  const cardDisplay = buildCardIntelligenceDisplay(
    suppressFormulaPlanning
      ? { ...intelligence, formulaComparison: null }
      : intelligence,
    {
    pricingAccepted,
    }
...
function SuggestedBudgetSplitRows({
  ...
  /** Apply is in flight — keep suggested amounts visible until acceptance commits. */
  ...
  const showDetailedPricingDetails = false;
  ...
  const writeActionLabel = isPricingRevertRow
    ? compactSuggestedActionLabel(block.lumpSumOnly)
    : isNationalComparison
  ...
  return (
    <View style={[styles.budgetSplitPanel, { borderTopColor: divider }]}>
      ...
      {showDetailedPricingDetails &&
      isFlooringLineCard &&
      flooringTransitionDisplayLines.length ? (
      ...
      {showDetailedPricingDetails && showFlooringBreakdownToggle ? (
        <>
          <TouchableOpacity
      ...
      {showDetailedPricingDetails && display.statusLine ? (
        <Text
          style={{
      ...
      {showDetailedPricingDetails && stageCoversLine ? (
        <Text
          style={{
            color: caption,
            fontSize: 12,
...
function ComparisonToggle({
  ...
  if (isNationalComparison) {
    // The card already shows its active/saved pricing. Keep the national
    // benchmark out of the primary Confirm Scope flow rather than appending a
    // second pricing choice underneath it.
    return null;
  }
...
const NotesScopeSelector = React.memo(function NotesScopeSelector({
  ...
  return (
    ...
    >
      ...
      <Text
        style={{
          color: darkMode ? 'rgba(245,247,250,0.72)' : Colors.text,
          fontSize: 11,
          lineHeight: 15,
          marginBottom: 4,
        }}
      >
        Your notes look like a plumbing bid. Choose the closest match — scope
        cards and measurements update below.
      </Text>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
        {(
          [
            ['plumbing', 'Room remodel'],
            ['plumbing_new_construction', 'New construction'],
            ['plumbing_service', 'Service / repair'],
            ['plumbing_bathroom', 'Bathroom'],
            ['plumbing_kitchen', 'Kitchen'],
            ['whole_project', 'Whole project / GC'],
...
function QuantitySection({
  ...
  if (itemId === 'window_install') {
    resolved = {
      ...
      unit: 'each',
      showInput: true,
      pricingReady: Number(resolved.quantity) > 0,
    };
  }
  if (
    ...
    const windowResolved = resolveChecklistItemQuantity(
      'window_install',
      norm,
      { templateKey, notes: originalNotes }
    );
    resolved = {
      ...resolved,
      quantity: windowResolved.quantity,
      unit: 'each',
  ...
  const electricalConversionBreakdown =
    itemId === 'electrical_rough' &&
    isExistingShellConversionJob(templateKey, undefined, originalNotes)
      ? existingShellConversionElectricalBreakdown(
  ...
  if (rule.dualAllowanceField) {
    ...
    if (resolved.pricingReady && !showEditor) {
      ...
      const dualLiveManualBlock: SuggestedPricingBlock | null =
        dualLiveManualTotal > 0
          ? {
              material: dualLiveMaterial || 0,
              labor: dualLiveLabor || 0,
              total: dualLiveManualTotal,
              ...
              basis:
                displayResolved.quantity != null && displayResolved.unit
                  ? {
                      quantity: displayResolved.quantity,
      ...
      const scopeMeasurementsRecord = measurementsInput as Record<
        ...
        unknown
      >;
      const noteBackedFlooring =
        itemId === 'flooring' &&
        /\b(?:install|installation|replace|replacement|new)\b[^.;\n]{0,80}\b(?:lvp|flooring|vinyl|laminate)\b/i.test(
          String(originalNotes || '')
        ) &&
        Number(measurementsInput.flooringSqft) > 0;
      const noteBackedDrywall =
        itemId === 'drywall' &&
        /\b(?:patch(?:ing)?|repair)\b[^.;\n]{0,30}\bdrywall\b|\bdrywall\b[^.;\n]{0,30}\b(?:patch(?:ing)?|repair)\b/i.test(
          String(originalNotes || '')
        ) &&
    ...
    if (!showEditor) {
      ...
      // Bathroom demo can have a resolved combined wall + floor quantity
      // before the generic tier reports pricingReady. Keep its amount visible
      // instead of showing only the quantity and planning-status label.
      const planningFill =
        ...
        !(Number(resolved.quantity) > 0)
          ? null
          : formulaPlanning.fill ||
            (itemId === 'demo' &&
            resolved.unit === 'sqft' &&
            Number(resolved.quantity) > 0
              ? resolveBathroomWetAreaDemoSuggestedPricing({
                  measurementsInput,
                  tileSqft: Number(resolved.quantity),
                  wallTileSqft:
                    Number(measurementsInput.showerWallTileSqft) || undefined,
                  panSqft:
                    Number(measurementsInput.showerFloorTileSqft) || undefined,
    ...
    ? itemId === 'paint_repair' &&
      ...
      ? (resolveBathroomPaintRepairMissingLabel({
          ...
          bathroomPaintRepairScopeSource:
            measurementsInput.bathroomPaintRepairScopeSource,
  ...
  const windowTrimQuantity =
    ...
    String(templateKey || '').toLowerCase() === 'bathroom'
      ? Number(resolved.quantity) || 0
      : 0;
  const windowTrimSuggested =
    windowTrimQuantity > 0
      ? {
          fill: {
            material: windowTrimQuantity * 45,
            labor: windowTrimQuantity * 135,
            total: windowTrimQuantity * 180,
            ...
            mode: 'suggested_price' as const,
            basis: { quantity: windowTrimQuantity, unit: 'each' },
            benchmarkAction: 'price_ready' as const,
            productionStatus: 'review_required' as const,
          },
          comparison: null,
        }
      : itemId === 'exterior_trim_paint' &&
  ...
  const wetAreaDemoFallback =
    ...
    Number(resolved.quantity) > 0
      ...
      : null;
  const standaloneBathroomPatchFill =
    itemId === 'patch_repair' &&
    String(templateKey || '').toLowerCase() === 'bathroom' &&
    Number(resolved.quantity) > 0
      ? resolveBathroomDrywallPatchSuggestedPricing({
      ...
      : null;
  const catalogSuggested = windowTrimSuggested
    ? windowTrimSuggested
    : standaloneBathroomPatchFill
    ? { fill: standaloneBathroomPatchFill, comparison: null }
    : assemblyInsulationPricing
    ? {
    ...
    : initialSuggestedFromCatalog.fill
      ...
      : wetAreaDemoFallback
        ? {
            fill: wetAreaDemoFallback,
            comparison: initialSuggestedFromCatalog.comparison,
          }
        : initialSuggestedFromCatalog;
  ...
  const liveManualBlock: SuggestedPricingBlock | null =
    liveManualTotal > 0
      ...
      : null;
  const userCommittedPricing = hasUserCommittedScopePricing(
    itemId,
    measurementsInput.itemQuantities || {},
    measurementsInput.pricingAcceptance
  );
  ...
  if (
    ...
    suggestedBudgetSplit = windowTrimSuggested?.fill ?? null;
    suggestedComparisonSplit = null;
  }
      ...
      >
        ...
        {(() => {
          ...
          const suggestedCardShowsQuantityLine = activeSuggestedPricingBlock
            ? confirmScopeSuggestedCardShowsQuantityLine({
                itemId,
                block: activeSuggestedPricingBlock,
                quantitySource: resolved.quantitySource,
                hasPrimaryTakeoff: hasPrimaryTakeoffForDisplay,
                livingSf: livingSfForPricingCard,
                confidenceLabel: intelligence?.pricing?.confidenceLabel,
              })
            : false;
          const acceptedDisplay = accepted
          ...
          const inlineSqftTakeoff = showInlineSqftTakeoff ? (
            <InlineTakeoffCountInput
              ...
              value={
                itemId === 'paint_repair'
                  ? String(
                      measurementsInput.itemQuantities?.patch_repair
                        ?.quantity ?? ''
                    )
                  : itemInput?.quantity ??
              ...
              onCommit={text => {
                onItemQuantityChange(
                  itemId === 'paint_repair' ? 'patch_repair' : itemId,
                  text,
                  'count',
                  resolved.unit || rule.defaultUnit,
                  'user_entered'
          ...
          const paintMeasurementInput =
            itemId === 'paint_repair' ? (
              <InlineTakeoffCountInput
                ...
                onBlur={() => blurQuantityField('interior_paint', 'count')}
                Colors={Colors}
                darkMode={darkMode}
                applying={applying}
              />
          ...
          return (
            <>
              ...
              {showInlineSqftTakeoff ? (
                ...
                resolved.quantity > 0 &&
                !suggestedCardShowsQuantityLine ? (
                <PricingAmountRow
              ...
              {electricalConversionBreakdownLabel ? (
                <Text
                  style={{
                    color: pricingLabelColor(darkMode, Colors),
                    fontSize: 11,
                ...
                >
                  Planning rollup: {electricalConversionBreakdownLabel}
                </Text>
              ) : null}
              {noteVarianceNotice}
              {!(
...
const BathroomCountStepper = React.memo(
  function BathroomCountStepper({
  label,
  value,
  onAdjust,
  max,
  disabled = false,
  diagnosticLocalOnly = false,
  applying,
  darkMode,
  Colors,
}: {
  ...
  max: number;
  disabled?: boolean;
  diagnosticLocalOnly?: boolean;
  applying: boolean;
  darkMode: boolean;
  Colors: ReturnType<typeof getColors>;
}) {
  const [displayValue, setDisplayValue] = useState(
    diagnosticLocalOnly ? 0 : value
  );
  const displayValueRef = useRef<number | null>(value);
  const onAdjustRef = useRef(onAdjust);
  onAdjustRef.current = onAdjust;
  const pendingValueRef = useRef<number | null | undefined>(undefined);
  const renderCountRef = useRef(0);
  const pressCountRef = useRef(0);
  renderCountRef.current += 1;
  const quantityProfilingEnabled =
    __DEV__ &&
    Boolean(
      (
        globalThis as typeof globalThis & {
          __BPS_PROFILE_QUANTITY__?: boolean;
        }
      ).__BPS_PROFILE_QUANTITY__
    );
  if (quantityProfilingEnabled) {
    console.debug(
      `[ConfirmScope] ${label} quantity row render #${renderCountRef.current}`
    );
  }

  useEffect(() => {
    if (diagnosticLocalOnly) return;
  ...
  }, [diagnosticLocalOnly, value]);

  const adjust = useCallback(
    (delta: number) => {
      if (diagnosticLocalOnly) {
        const startedAt = performance.now();
        setDisplayValue(previous => {
          const next = Math.max(0, Math.min(max, (previous ?? 0) + delta));
          displayValueRef.current = next;
          return next;
        });
        if (quantityProfilingEnabled) {
          console.debug(
            `[ConfirmScope diagnostic] ${label} local press ${
      ...
      if (
        ...
        disabled ||
        (delta < 0 && !current) ||
        (delta > 0 && current != null && current >= max)
      ) {
        return;
      }
      const pressId = ++pressCountRef.current;
      const next =
        (current ?? 0) + delta < 1
          ? null
          : Math.min(max, (current ?? 0) + delta);
      displayValueRef.current = next;
      pendingValueRef.current = next;
      setDisplayValue(previous => (previous === next ? previous : next));
      if (quantityProfilingEnabled) {
        console.time(`confirm-scope quantity press ${pressId}`);
        onAdjustRef.current(delta);
        console.timeEnd(`confirm-scope quantity press ${pressId}`);
      } else {
        onAdjustRef.current(delta);
      }
    },
    [
      ...
      label,
      max,
      quantityProfilingEnabled,
    ]
  );

  const unavailable = applying || disabled;
  const minusDisabled = unavailable || !displayValue;
  const plusDisabled =
    unavailable || (displayValue != null && displayValue >= max);
    ...
    >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        <Pressable
          ...
          // Keep adjacent stepper rows from sharing a vertical hit target.
  ...
  (previous, next) =>
    ...
    previous.diagnosticLocalOnly === next.diagnosticLocalOnly &&
...
function YesNoRow({
  ...
  onBathroomPaintRepairSeverityChange,
  ...
  onKitchenBacksplashDemoDifficultyChange,
  scopeChecklistItems,
  suppressSuggestedPricing = false,
  visualCtx,
  Colors,
  ...
  onBathroomPaintRepairSeverityChange?: (
    severity: BathroomPaintRepairSeverity | null
  ) => void;
  onBathroomDrywallPaintCombinedAssemblyChange?: (
  ...
  const paintRepairQuantitySource =
    measurementsInput.itemQuantities?.patch_repair?.quantitySource ??
    measurementsInput.itemQuantities?.paint_repair?.quantitySource;
  const notesDescribePatchRepair =
    /\b(?:patch(?:ing)?|repair(?:s|ed|ing)?|drywall|texture|skim\s*coat)\b/i.test(
      String(originalNotes || '')
    );
  const explicitPaintRepairSqft =
    paintRepairQuantitySource === 'user_entered' || notesDescribePatchRepair
      ? userPaintRepairSqft
      : 0;
  ...
  const paintRepairAutoFlow =
    ...
    bathroomPaintRepairUsesAutoFlow({
      ...
      bathroomFloorSqft: measurementsInput.bathroomFloorSqft,
      enteredTakeoffSqft:
        explicitPaintRepairSqft > 0 ? explicitPaintRepairSqft : null,
      scopeSource: measurementsInput.bathroomPaintRepairScopeSource,
    });
  const storedPaintRepairSeverity =
    measurementsInput.bathroomPaintRepairSeverity ?? null;
  const displayedPaintRepairSeverity = resolveBathroomPaintRepairSeverity(
    storedPaintRepairSeverity
  );
  ...
  const entireRoomPaintSqft =
    resolvedPaintRepairScope === 'full_room' && explicitPaintRepairSqft > 0
      ? explicitPaintRepairSqft
      : 0;
  const patchRepairSqft =
    (simpleBathroomPaintRepair || resolvedPaintRepairScope === 'affected_area') &&
    (simpleBathroomPaintRepair
      ? explicitPaintRepairSqft
      : explicitPaintRepairSqft) > 0
      ? simpleBathroomPaintRepair
        ? explicitPaintRepairSqft
        : explicitPaintRepairSqft
      : 0;
  ...
  const separateLinesPaintFill = useMemo(() => {
    ...
    const paintSqftForPricing =
      Number(measurementsInput.paintAreaSqft) ||
      Number(measurementsInput.wallPaintSqft) ||
      Number(measurementsInput.itemQuantities?.interior_paint?.quantity) ||
      null;
    return (
      resolveBathroomPaintRepairSuggestedPricing({
        checklistItems: scopeChecklistItems,
        patchSqft: patchRepairSqft > 0 ? patchRepairSqft : null,
        paintSqft: paintSqftForPricing,
        showerWallTileSqft:
          Number(measurementsInput.showerWallTileSqft) || null,
        paintRepairScope: storedPaintRepairScope,
        paintRepairEntireRoom: measurementsInput.bathroomPaintRepairEntireRoom,
    ...
    measurementsInput.paintAreaSqft,
    measurementsInput.wallPaintSqft,
    measurementsInput.itemQuantities?.interior_paint?.quantity,
    measurementsInput.bathroomPaintRepairEntireRoom,
    measurementsInput.bathroomInteriorPaintMobilization,
  ...
  const showBacksplashDemoDifficultyPrompt =
    item.id === 'backsplash_demo' &&
    displayedState === 'included' &&
    String(templateKey || '').toLowerCase() === 'kitchen';
  const backsplashDemoDifficultyApplied = Boolean(
    measurementsInput.pricingAcceptance?.backsplash_demo
  );
  const [backsplashDemoPromptExpanded, setBacksplashDemoPromptExpanded] =
    useState(false);
  const showBacksplashDemoQuestions =
    showBacksplashDemoDifficultyPrompt &&
    (!backsplashDemoDifficultyApplied || backsplashDemoPromptExpanded);
  const showBacksplashDemoCollapsed =
    showBacksplashDemoDifficultyPrompt &&
    backsplashDemoDifficultyApplied &&
    !backsplashDemoPromptExpanded;
  const storedBacksplashDemoDifficulty = resolveKitchenBacksplashDemoDifficulty(
    measurementsInput.kitchenBacksplashDemoDifficulty
  );
  const displayedBacksplashDemoDifficulty =
    ...
    >
      ...
      {showPaintRepairSeverityPrompt ? (
        <View style={{ marginTop: 10 }}>
          ...
          >
            How much patchwork is expected?
          </Text>
          <View style={styles.choiceWrap}>
            {BATHROOM_PAINT_REPAIR_SEVERITY_OPTIONS.map(opt => {
              const active = displayedPaintRepairSeverity === opt.id;
              return (
                <TouchableOpacity
                  key={opt.id}
                  activeOpacity={0.88}
                  onPress={() => {
                    hapticTap();
                    onBathroomPaintRepairSeverityChange?.(
                      active && storedPaintRepairSeverity === opt.id
                        ? null
                        : opt.id
                    );
                  }}
          ...
          >
            {
              BATHROOM_PAINT_REPAIR_SEVERITY_OPTIONS.find(
                opt => opt.id === displayedPaintRepairSeverity
              )?.helper
            }
          </Text>
        </View>
      ...
      {showBacksplashDemoCollapsed ? (
        <TouchableOpacity
          activeOpacity={0.75}
          onPress={() => {
            hapticTap();
            setBacksplashDemoPromptExpanded(true);
          }}
          style={{ marginTop: 10 }}
        >
          <View
          ...
          >
            ...
            >
              {KITCHEN_BACKSPLASH_DEMO_DIFFICULTY_OPTIONS.find(
                opt => opt.id === displayedBacksplashDemoDifficulty
              )?.label || 'Removal difficulty selected'}
            </Text>
            <Text style={{ color: '#0f766e', fontSize: 12, fontWeight: '700' }}>
              Edit
            </Text>
      ...
      {showBacksplashDemoQuestions ? (
        <View style={{ marginTop: 10 }}>
          ...
          >
            How difficult is the backsplash removal?
          </Text>
          <View style={styles.choiceWrap}>
            {KITCHEN_BACKSPLASH_DEMO_DIFFICULTY_OPTIONS.map(opt => {
              const active = displayedBacksplashDemoDifficulty === opt.id;
              return (
                <TouchableOpacity
                  key={opt.id}
                  activeOpacity={0.88}
                  onPress={() => {
                    hapticTap();
                    onKitchenBacksplashDemoDifficultyChange?.(
                      active && storedBacksplashDemoDifficulty === opt.id
                        ? null
                        : opt.id
                    );
                  }}
          ...
          >
            Drywall patch or replacement is priced on a separate line.
          </Text>
        </View>
      ) : null}
      {isCustom ? (
...
function ChoiceRow({
  ...
  const handleLocalChoice = useCallback(
    (choiceId: string) => {
      ...
      // Paint the optimistic choice before the parent reconciles the full
      // scope/pricing tree.
      requestAnimationFrame(() => onSelectRef.current(choiceId));
    },
    [item.choiceId]
  );
...
const INSULATION_LOCATION_OPTIONS = [
  ...
  { key: 'floor', label: 'Floor insulation' },
] as const;
...
const QuickMeasurementField = React.memo(function QuickMeasurementField({
  ...
  value: string;
  fromNotes?: boolean;
  /** calm = detected/confirmed (no badge); needs_confirmation = needs entry; suggestion = compact Use row */
  ...
  const handleInputChange = (nextValue: string) => {
    ...
    if (usesNumericEditDraft) {
      ...
      // Baseboard LF and patch sqft fan out to linked scope quantities.
      // Commit it during typing so that the linked quantity reconciliation
      // cannot replace the visible draft before blur.
      if (
  ...
  // Match renderDemoSqftField shells so shower floor reads like wall/bath floor inputs.
      ...
      >
        ...
        <View
          style={[
            ...
            {
              borderColor: measurementInputShell.borderColor,
              backgroundColor: measurementInputShell.backgroundColor,
            },
          ]}
        >
          <TextInput
...
function buildInsulationAssemblyRows(
  ...
  const numberFor = (key: string) => {
    const value = Number(String(measurements[key] ?? '').replace(/,/g, ''));
    return Number.isFinite(value) && value > 0 ? value : 0;
  };
  const wallSqft = numberFor('exteriorWallInsulationSqft');
  const atticSqft = numberFor('atticInsulationSqft');
  ...
  const isLegacyUniformBattSeed =
    storedAssemblies.length === 3 &&
    wallSqft > 0 &&
    atticSqft > 0 &&
    floorSqft > 0 &&
    storedAssemblies.every(
      row =>
        insulationMaterialTypeKey(row.materialType) === 'batt' &&
        row.rValue.trim().toLowerCase() === 'r-21' &&
        Number(String(row.sqft ?? '').replace(/,/g, '')) === wallSqft
    );
  if (isLegacyUniformBattSeed) {
    return [
      {
        id: 'insulation-assembly-migrated-wall',
        materialType: 'Batt',
        rValue: 'R-21',
        sqft: wallSqft,
      ...
      {
        id: 'insulation-assembly-migrated-attic',
      ...
      {
        id: 'insulation-assembly-migrated-floor',
        materialType: 'Batt',
        rValue: 'R-30',
        sqft: floorSqft,
        location: 'floor',
...
function InsulationAssemblyCard({
  ...
  const skipAssemblySyncRef = useRef(false);
  ...
  useEffect(() => {
    if (skipAssemblySyncRef.current) {
      skipAssemblySyncRef.current = false;
      return;
    }
  ...
  const updateRows = (next: InsulationAssembly[]) => {
    skipAssemblySyncRef.current = true;
    setRows(next);
  ...
  const toggleType = (materialType: string) => {
    ...
    const hasCompleteMatchingRow = matching.some(
      row => row.materialType.trim() && row.rValue.trim() && Number(row.sqft) > 0
    );
    if (matching.length && hasCompleteMatchingRow) {
    ...
    if (insulationMaterialTypeKey(materialType) === 'blown-in') {
      const atticRow =
        rows.find(
          row =>
            row.location === 'attic_ceiling' &&
            insulationMaterialTypeKey(row.materialType) !== 'blown-in'
        ) || rows.find(row => row.location === 'attic_ceiling');
      if (atticRow) {
        const atticSqft = Number(
          String(measurements.atticInsulationSqft ?? '').replace(/,/g, '')
        );
        const next = rows
          .filter(row => !matching.some(match => match.id === row.id) || row.id === atticRow.id)
          .map(row =>
            row.id === atticRow.id
              ? {
                  ...row,
                  ...
                  rValue: 'R-38',
                  sqft:
                    Number.isFinite(atticSqft) && atticSqft > 0
                      ? atticSqft
                      : row.sqft,
        ...
        setExpandedAssemblyId(atticRow.id);
        updateRows(next);
        return;
      }
    }
...
function CollapsibleQuickMeasurements({
  ...
  qmFlushRef?: React.MutableRefObject<(() => void) | null>;
}) {
  ...
  const rows = useMemo(() => {
    ...
    if (String(effectiveTemplateKey || '').toLowerCase() === 'insulation') {
      ...
      const noteHasField = (key: QuickMeasurementFieldKey) =>
        noteQuickMeasurements.keys instanceof Set
          ? noteQuickMeasurements.keys.has(key)
          : Array.isArray(noteQuickMeasurements.keys)
            ? noteQuickMeasurements.keys.includes(key)
            : false;
      const hasRoofDeckReference =
        noteHasField('insulatedRoofDeckSqft') ||
      ...
      const hasOpeningDeductionReference =
        noteHasField('openingDeductionSqft') ||
      ...
      return baseRows
        .map(row =>
          row.filter(field => {
            if (
              hiddenInsulationFields.has(field.key) ||
              !insulationFields.has(field.key)
            ) {
              return false;
            }
            if (
              field.key === 'insulatedRoofDeckSqft' &&
              !hasRoofDeckReference
            ) {
              return false;
            }
            if (
              field.key === 'openingDeductionSqft' &&
              !hasOpeningDeductionReference
            ) {
              return false;
            }
            return true;
  ...
  const groups = useMemo(() => {
    ...
    const stickyMeasurementKeys = new Set<QuickMeasurementFieldKey>([
      'wallPaintSqft',
      'paintAreaSqft',
      'patchRepairSqft',
      'baseboardLf',
    ...
    // Keep the three linked interior-finish measurements together while their
    // values are being edited. Their relevance can change on each keystroke,
    // but the input must not jump between sections or remount.
    const stickyResults: QuickMeasurementFieldResult[] = [];
    ([
      'wallPaintSqft',
      'paintAreaSqft',
      'baseboardLf',
      'patchRepairSqft',
    ] as QuickMeasurementFieldKey[]).forEach(key => {
      (Object.keys(positioned) as QuickMeasurementGroupId[]).forEach(
        groupId => {
          const index = positioned[groupId].findIndex(
  ...
  const readStepperCountsFromMeasurements = useCallback(
  ...
  const [stepperCounts, setStepperCounts] = useState<WetAreaStepperCounts>(
    readStepperCountsFromMeasurements
  );
  ...
  const existingCountsRef = useRef(existingCounts);
  existingCountsRef.current = existingCounts;
  const demoCountsRef = useRef(demoCounts);
  demoCountsRef.current = demoCounts;
  const [reuseExistingShowerDoor, setReuseExistingShowerDoor] = useState(() =>
    Boolean(measurements.reuseExistingShowerDoor)
  );
  const BATHROOM_QM_PARENT_FLUSH_MS = 150;
  const bathroomParentFlushTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null
  );
  const wetAreaCommitOptionsRef = useRef<{ keepingExisting?: boolean } | undefined>(
    undefined
  );
  const bathroomFixturesQmFlushRef = useRef<QmBathroomFixturesPanelsHandle | null>(
    null
  );
  ...
  const stepperScheduleTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null
  );
  ...
  useEffect(
    () => () => {
      ...
      if (stepperScheduleTimerRef.current) {
        clearTimeout(stepperScheduleTimerRef.current);
      }
      if (bathroomParentFlushTimerRef.current) {
        clearTimeout(bathroomParentFlushTimerRef.current);
      }
    },
    []
  );
  useEffect(() => {
    ...
    const next = readStepperCountsFromMeasurements();
    latestStepperRef.current = next;
    setStepperCounts(next);
  }, [
    readStepperCountsFromMeasurements,
    measurements.bathCount,
    measurements.tilePanBathCount,
  ...
  const flushBathroomQmToParent = useCallback(() => {
    if (bathroomParentFlushTimerRef.current) {
      clearTimeout(bathroomParentFlushTimerRef.current);
      bathroomParentFlushTimerRef.current = null;
    }
    const latest = latestExistingDemoRef.current;
    const stepper = latestStepperRef.current;
    const options = wetAreaCommitOptionsRef.current;
    ...
    startTransition(() => {
      setMeasurements(prev => {
        ...
        if (stepper.showerDoorCount != null && stepper.showerDoorCount > 0) {
          itemQuantities.glass_door = {
            quantity: String(stepper.showerDoorCount),
            unit: 'each',
            quantitySource: 'user_entered',
          };
        } else {
        ...
        return {
          ...
          ...latest.existing,
          ...latest.demo,
          bathCount: stepper.bathCount,
          tilePanBathCount: stepper.tilePanBathCount,
          prefabBathCount: stepper.prefabBathCount,
          prefabEnclosureBathCount: stepper.prefabEnclosureBathCount,
          tubBathCount: stepper.tubBathCount,
          bathFloorTileCount: stepper.bathFloorTileCount,
          showerDoorCount: stepper.showerDoorCount,
          wetAreaFinish,
          reuseExistingShowerDoor: latest.reuseExistingShowerDoor,
          demoWetAreaManualOverrides: demoOverridesRef.current,
      ...
      onWetAreaSteppersChange?.(stepper, options);
      onShowerDoorCountChange?.(stepper.showerDoorCount);
      onWetAreaExistingDemoChange?.({
        existing: latest.existing,
        demo: latest.demo,
        reuseExistingShowerDoor: latest.reuseExistingShowerDoor,
        demoOverrides: demoOverridesRef.current,
      });
    });
  }, [
    effectiveTemplateKey,
  ...
  const scheduleBathroomParentFlush = useCallback(() => {
    if (bathroomParentFlushTimerRef.current) {
      clearTimeout(bathroomParentFlushTimerRef.current);
    }
    bathroomParentFlushTimerRef.current = setTimeout(() => {
      bathroomParentFlushTimerRef.current = null;
      flushBathroomQmToParent();
    }, BATHROOM_QM_PARENT_FLUSH_MS);
  }, [flushBathroomQmToParent]);

  useEffect(() => {
    if (!qmFlushRef) return;
    qmFlushRef.current = () => {
      flushBathroomQmToParent();
      bathroomFixturesQmFlushRef.current?.flushPending();
    };
    return () => {
      qmFlushRef.current = null;
    };
  }, [qmFlushRef, flushBathroomQmToParent]);
  ...
  const scheduleExistingDemoCommit = useCallback(
    (
      ...
      // Use the optimistic row state as the override source. Reading only
      // from the parent measurements here replays stale note-inferred counts
      // and makes a tap on one stepper appear to toggle a sibling row.
      const stored = {
        ...readWetAreaDemoCounts(measurements),
        ...demoCountsRef.current,
      };
      if (demoOverride) {
      ...
      demoCountsRef.current = merged;
      setDemoCounts(merged);
  ...
  const scheduleWetAreaCommit = useCallback(
    (
      ...
      if (gen !== stepperGenRef.current) return;
      latestStepperRef.current = next;
      wetAreaCommitOptionsRef.current = options;
      // Update the visible scope cards from the optimistic QM state now.
      // The parent measurement flush remains debounced below so pricing and
      // persistence do not block the tap feedback.
      onWetAreaSteppersChange?.(next, options);
  ...
  const adjustStepperCount = useCallback(
    (key: keyof WetAreaStepperCounts, delta: number) => {
      ...
      const current = latestStepperRef.current[key] ?? 0;
      const cleaned = clampBathCount(
        current + delta < 1 ? null : current + delta,
        wetAreaStepperMax
      );
      const next = { ...latestStepperRef.current, [key]: cleaned };
  ...
  const adjustTileBathCount = useCallback(
    (delta: number) => {
      const gen = ++stepperGenRef.current;
      const current = latestStepperRef.current.bathCount ?? 0;
      const cleaned = clampBathCount(
        current + delta < 1 ? null : current + delta,
        wetAreaStepperMax
      );
      const next = { ...latestStepperRef.current, bathCount: cleaned };
      latestStepperRef.current = next;
      setStepperCounts(next);
  ...
  const adjustExistingCount = useCallback(
    (key: keyof WetAreaExistingCounts, delta: number) => {
      ...
      const current = existingCountsRef.current[key] ?? 0;
      const cleaned = clampBathCount(
        current + delta < 1 ? null : current + delta,
        wetAreaStepperMax
      );
      const next = { ...existingCountsRef.current, [key]: cleaned };
      existingCountsRef.current = next;
      setExistingCounts(next);
      scheduleExistingDemoCommit(
        next,
        buildInstallCounts(),
        keepingExistingWetArea,
  ...
  const adjustDemoCount = useCallback(
    (key: WetAreaDemoOverrideKey, delta: number) => {
      const gen = ++existingDemoGenRef.current;
      const current = demoCountsRef.current[key] ?? 0;
      const cleaned = clampBathCount(
        current + delta < 1 ? null : current + delta,
        wetAreaStepperMax
      );
      // Once a contractor edits one demo row, freeze the other visible demo
      // rows for this panel too. Re-running note inference must not make a
      // tub tap appear to toggle the tile-pan row (or any sibling row).
      const frozenOverrides = Object.keys(demoCountsRef.current).reduce(
        (next, demoKey) => {
          next[demoKey as WetAreaDemoOverrideKey] = true;
          return next;
        },
        {
          ...demoOverridesRef.current,
        } as Partial<Record<WetAreaDemoOverrideKey, boolean>>
      );
      demoOverridesRef.current = frozenOverrides;
      const next = { ...demoCountsRef.current, [key]: cleaned };
      demoCountsRef.current = next;
      setDemoCounts(next);
      scheduleExistingDemoCommit(
        existingCountsRef.current,
        buildInstallCounts(),
        keepingExistingWetArea,
        reuseExistingShowerDoor,
        gen,
  ...
  const showGarageDoorSteppers =
    wholeHomeLayout && !isGarageConversionJob(projectType, notes);
  ...
  const renderDemoSqftField = (
    ...
    options?: {
      syncExistingWall?: boolean;
      syncExistingFloor?: boolean;
    }
  ) => {
    const value = String(measurements[field] ?? '');
    const applySqftChange = (cleaned: string) => {
      setMeasurements(prev => {
        const itemQuantities = { ...(prev.itemQuantities || {}) };
        ...
        if (field === 'showerWallTileSqft' || field === 'showerFloorTileSqft') {
          ...
          const nextWallSqft =
            field === 'showerWallTileSqft'
              ? cleaned
              : prev.showerWallTileSqft;
          const nextFloorSqft =
            field === 'showerFloorTileSqft'
              ? cleaned
              : prev.showerFloorTileSqft;
          const showerDemoSqft =
            Number(String(nextWallSqft || '').replace(/,/g, '')) +
            Number(String(nextFloorSqft || '').replace(/,/g, ''));
          if (showerDemoSqft > 0) {
            itemQuantities.demo__sqft_basis = {
              quantity: showerDemoSqft,
              unit: 'sqft',
              quantitySource: 'user_entered',
            };
          } else {
            delete itemQuantities.demo__sqft_basis;
          }
        }
      ...
      if (!bathroomPhotoWetArea) return;
      const sqftActive = Number(String(cleaned).replace(/,/g, '')) > 0;
      let nextExisting = { ...existingCounts };
      if (options?.syncExistingWall) {
        nextExisting = {
          ...nextExisting,
          existingTileWallCount: sqftActive ? 1 : null,
        };
        setExistingCounts(nextExisting);
      }
      if (options?.syncExistingFloor) {
        nextExisting = {
          ...nextExisting,
          existingBathFloorTileCount: sqftActive ? 1 : null,
        };
        setExistingCounts(nextExisting);
      }
      const demoOverride =
        field === 'showerWallTileSqft'
          ? ({
              key: 'demoTileWallCount' as WetAreaDemoOverrideKey,
              value: sqftActive ? 1 : null,
            } as const)
          : field === 'bathroomFloorSqft'
            ? ({
                key: 'demoBathFloorTileCount' as WetAreaDemoOverrideKey,
                value: sqftActive ? 1 : null,
              } as const)
            : undefined;
      const gen = ++existingDemoGenRef.current;
  ...
  const renderKeepingExistingWetAreaToggle = () => (
    <View style={{ marginBottom: 10, marginTop: 2 }}>
      ...
      >
        ...
        <View
          style={{
            borderTopWidth: 1,
            borderTopColor: keepingExistingWetArea
        ...
        >
          ...
          >
            {keepingExistingWetArea ? 'Selected · tap to clear' : 'Select'}
          </Text>
        </View>
      </TouchableOpacity>
    </View>
  ...
  const renderWetAreaInstallSubheading = (label: string) => (
    <Text
      style={{
        color: captionColor(darkMode, Colors),
        fontSize: 10,
  ...
  const renderExistingWetAreaPanel = () => {
    const existingPanelStyle = qmExistingScopePanelStyle(darkMode);
    return (
      <View
        style={[
      ...
      >
        ...
        {renderDemoSqftField(
          'Existing shower wall tile',
          'Wall tile area in the current shower — sqft, not a fixture count.',
          'showerWallTileSqft',
          { syncExistingWall: true }
        )}
        {renderBathCountStepper(
        ...
        {renderDemoSqftField(
          'Existing bath floor tile',
          'Bathroom floor finish area — separate from shower pan tile.',
          'bathroomFloorSqft',
          { syncExistingFloor: true }
        )}
      </View>
    );
  };
    ...
    (key: QuickMeasurementFieldKey, value: string) => {
      ...
      setMeasurements(prev => {
        ...
        > = {
          ...
          patchRepairSqft: { id: 'patch_repair', unit: 'sqft' },
        ...
        // Paint area is the base painting quantity, not localized patch/repair
        // area. Do not mirror it into the separate paint_repair takeoff row.
        if (key === 'paintAreaSqft') {
          const existingPatch = nextItemQuantities.paint_repair;
          if (existingPatch?.quantitySource !== 'user_entered') {
            delete nextItemQuantities.paint_repair;
          }
        }
        ...
        measurementsRef.current = committedMeasurements;
  ...
  const endEditingField = useCallback((key: QuickMeasurementFieldKey) => {
    ...
    // Keep the field in its home section after blur — never drop confirmed
    // values into the Confirmed block below (layout jump on every scope).
    if (home.homeGroup != null && home.homeIndex != null) {
      setTypedMeasurementHomes(prev =>
        prev[key]
          ? prev
          : {
  ...
  const renderProjectComplexityPanel = (
    ...
    const panelStyle = qmNeutralScopePanelStyle(darkMode);
    const bathRemodelPlumbing =
      trade === 'plumbing' &&
      measurements.plumbingWorkflowMode === 'bathroom_remodel';
    const showLivingAreaComplexity =
      trade === 'electrical' ||
      plumbingWorkflowUsesLivingAreaComplexity(
        measurements.plumbingWorkflowMode
      );
    const helperCopy =
      trade === 'electrical'
        ...
        : bathRemodelPlumbing
          ? 'Multi-story homes add labor on rough-in, trim, and vertical runs. Material stays at base rates.'
    ...
    return (
      ...
      >
        ...
        {showLivingAreaComplexity && mepPlanImportStoryOnly ? (
          <TouchableOpacity
          ...
          {String(templateKey || '').toLowerCase() === 'painting' ? (
            ...
            >
              ...
              {(
                ...
                (surface === 'trim' &&
                  String(templateKey || '').toLowerCase() === 'bathroom' &&
                  /\bbaseboards?\b/i.test(notes || ''))
                ||
                (surface === 'trim' &&
                  includedScopeKeys.includes('baseboard_install'))
              ) ? (
...
export default function AIEstimateScopeAssumptionsModal({
  ...
  const bathroomQmFlushRef = useRef<(() => void) | null>(null);
  ...
  useEffect(() => {
    ...
    if (scopeDisplayReady) return;
    if (visible && hydratedVisibleSessionRef.current) {
  ...
  }, [visible, scopeHydrated, scopeDisplayReady]);

  const setMeasurementsSynced = useCallback(
    (update: React.SetStateAction<ScopeMeasurementsInputExtended>) => {
      const syncOptions = measurementsSyncOptionsRef.current;
      measurementsSyncOptionsRef.current = {};
      const skipPricingResync = Boolean(syncOptions.skipPricingResync);
  ...
  const scopeMeasurementsPayloadForCurrentState = useCallback(() => {
    ...
    if (draft) {
      measurements = applyNoteBackedBathroomConfirmScopePricing(
        draft,
        measurements,
    ...
    return reconciled;
  }, [checklist?.templateKey, scopeNotes, draft, displayItems, items]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      sourceItems = [
        ...
        ...(parsedInsulationNotes.airSealingIncluded
          ? [
              {
                id: 'air_sealing',
                inputType: 'yes_no' as const,
    ...
    if (effectiveTemplateKey === 'insulation' && !singleTradePlanImport) {
      const parsedAssemblies = parseInsulationAssembliesFromNotes(
    ...
    normalized = syncWindowInstallScopeFromNotes(normalized, {
      notes: scopeNotes,
      windowCount: nextMeasurements.windowCount,
      templateKey: effectiveTemplateKey,
    });
    ...
    if (
      ...
      const baseboardInstall = normalized.find(
        item => item.id === 'baseboard_install'
      );
      normalized.push({
        id: 'trim_paint',
        inputType: 'yes_no',
        label: 'Baseboard prep & paint',
        helperText:
          'Prep and paint the baseboards using the same LF takeoff as installation.',
        category: 'paint',
        state: baseboardInstall?.state === 'included' ? 'included' : 'unsure',
        noteBacked: baseboardInstall?.noteBacked,
      });
    }
    ...
    normalized = filterRoomRemodelNoteScopeItems(
      normalized,
      scopeNotes
    );
  ...
  useEffect(() => {
    ...
    setItems(prev => {
      const withDefaults = applyAdditionConversionScopeDefaults(prev, {
        templateKey: checklist.templateKey,
        projectType: draft?.projectType,
        notes: scopeNotes,
      });
      ...
      if (
        ...
        synced.every(item => {
          const prior = prev.find(row => row.id === item.id);
          return (
            prior &&
            prior.state === item.state &&
            prior.choiceId === item.choiceId
          );
        })
      ) {
        return prev;
  ...
  const handleBathroomPaintRepairSeverityChange = useCallback(
    (severity: BathroomPaintRepairSeverity | null) => {
      setMeasurementsSynced(prev => {
        const pricingAcceptance = { ...(prev.pricingAcceptance || {}) };
        delete pricingAcceptance.paint_repair;
        ...
        for (const key of Object.keys(itemQuantities)) {
          if (key === 'paint_repair' || key.startsWith('paint_repair__')) {
            delete itemQuantities[key];
          }
        }
        const wallSf = parseBathroomWallPaintSqft(prev.wallPaintSqft);
        if (wallSf != null && wallSf > 0) {
          itemQuantities.paint_repair = {
        ...
        return {
          ...prev,
          bathroomPaintRepairSeverity: severity,
          bathroomPaintRepairSeveritySource: severity ? 'user_selected' : null,
          pricingAcceptance,
          itemQuantities,
        };
      });
  ...
  const handleKitchenBacksplashDemoDifficultyChange = useCallback(
    (difficulty: KitchenBacksplashDemoDifficulty | null) => {
      setMeasurementsSynced(prev => {
        const pricingAcceptance = { ...(prev.pricingAcceptance || {}) };
        delete pricingAcceptance.backsplash_demo;
        return {
          ...prev,
          kitchenBacksplashDemoDifficulty: difficulty,
          kitchenBacksplashDemoDifficultySource: difficulty
            ? 'user_selected'
            : null,
          pricingAcceptance,
        };
  ...
  const qmScopeEmbeddedInQuickMeasurements = useCallback(
    (itemId: string) => {
      ...
      const namedKitchenApplianceHookup =
        String(checklist?.templateKey || '').toLowerCase() === 'kitchen' &&
        itemId === 'appliances' &&
        items.some(
          item =>
            item.state === 'included' &&
            /^electrical_(?:range|dishwasher|microwave|refrigerator|oven)_hookup$/.test(
              String(item.id || '')
            )
        );
      if (namedKitchenApplianceHookup) return true;
      if (
        shouldHideBathroomFixtureScopeCardInQmEmbed(
          itemId,
          measurements as Record<string, unknown>,
    ...
    const groups = sourceGroups
      .map(group => ({
        ...
        items: group.items.filter(
          item =>
            !(isBathroomFlow && item.state === 'excluded') &&
  ...
  useEffect(() => {
    ...
    measurements.concreteSubgradePrepSqft,
    measurements.concreteReinforcementSqft,
    measurements.complexFormingLf,
    measurements.concreteAreaByType,
    measurements.concreteThicknessInches,
    measurements.concreteThicknessByType,
  ]);
  ...
  useEffect(() => {
    ...
    if (templateKey !== 'kitchen') return;
    startTransition(() => {
      setItems(prev =>
        syncKitchenQmScopeItems(prev, measurements as Record<string, unknown>)
      );
    });
  }, [
    checklist?.templateKey,
  ...
  useEffect(() => {
    setItems(prev =>
      syncInteriorPaintScopeItems(prev, {
        ...
        patchRepairSqft:
          measurements.itemQuantities?.patch_repair?.quantity ??
          measurements.patchRepairSqft,
  ...
  useEffect(() => {
    ...
    setItems(prev =>
      syncWetAreaDemoScopeItems(prev, {
        ...
        demoShowerDoorCount: measurements.demoShowerDoorCount,
        installShowerDoorCount: measurements.showerDoorCount,
        demoOverrides: measurements.demoWetAreaManualOverrides,
        showerWallTileSqft: measurements.showerWallTileSqft,
        bathroomFloorSqft: measurements.bathroomFloorSqft,
      })
    );
  ...
  computeUnconfirmedSuggestedPricingRef.current = async (
    ...
    const bathroomPaintRepairScopeSelected =
      ...
      isPaintRepairScopeReady({
        ...
        enteredTakeoffSqft:
          parseScopeMeasurementInput(
            String(measurements.itemQuantities?.paint_repair?.quantity ?? '')
          ) ?? null,
      });
  ...
  const scopeItemsNeedingConfirmation = useMemo(
    () =>
      listScopeItemsNeedingConfirmation(displayItems, normMeasurements, {
        ...
        bathroomPaintRepairScopeSource:
          measurements.bathroomPaintRepairScopeSource,
        wallPaintSqft: measurements.wallPaintSqft,
        bathroomFloorSqft: measurements.bathroomFloorSqft,
        enteredTakeoffSqft:
          parseScopeMeasurementInput(
    ...
    [
      ...
      measurements.wallPaintSqft,
      measurements.bathroomFloorSqft,
      measurements.itemQuantities?.paint_repair?.quantity,
      measurements.bathroomToiletRelocateFloorType,
      measurements.bathroomVanityCountertopMaterialType,
    ]
  );
          ...
          ? (() => {
              ...
              if (!card || card.unit === 'allowance') {
                if (
                  ...
                  return {
                    kitchenFloorSqft: quantity.replace(/,/g, ''),
                    quickMeasurementSources: {
                      ...(prev.quickMeasurementSources || {}),
                      kitchenFloorSqft: 'user_entered' as const,
                    },
                  };
                }
                return {};
  ...
  const handleClearSuggestedPrefill = useCallback(
    (
      ...
      setMeasurementsSynced(prev => {
        ...
        if (
          !prev.pricingAcceptance?.[itemId] &&
          pendingUpdates?.some(
            update =>
              update.quantitySource === 'user_entered' &&
    ...
    const row =
      ...
      useConcreteLineCard ? (
        ...
        <ChoiceRow
          ...
          onBathroomPaintRepairSeverityChange={
            handleBathroomPaintRepairSeverityChange
          }
          onBathroomDrywallPaintCombinedAssemblyChange={
          ...
          onKitchenBacksplashDemoDifficultyChange={
            handleKitchenBacksplashDemoDifficultyChange
          }
  ...
  const scopeFooterLinkRowCount =
    (isElectricalConfirmScope && quickMeasurementsOpen ? 1 : 0) +
    (pricingFooterReady &&
    (scrollToPricingLabel ||
      ...
      ? 1
      : 0);
  /** Breathing room so the last scope card clears the pinned footer chrome. */
  const scopeScrollContentPadBottom = 24 + scopeFooterLinkRowCount * 28;
          ...
          <CollapsibleQuickMeasurements
            ...
            setMeasurements={
              ...
              'electrical'
                ...
                : update => {
                    // Bathroom steppers are high-frequency UI edits. Do not
                    // synchronously resync every applied pricing card for
                    // each tap; Confirm Scope performs the authoritative
                    // pricing reconciliation after the measurements settle.
                    if (
                      String(checklist?.templateKey || '').toLowerCase() ===
                      'bathroom'
                    ) {
                      measurementsSyncOptionsRef.current = {
                        skipPricingResync: true,
                      };
                    }
                    setMeasurementsSynced(update);
                  }
            }
            templateKey={checklist?.templateKey}
            projectType={draft?.projectType}
            ...
            onWetAreaExistingDemoChange={({
              ...
              setItems(prev =>
                syncWetAreaDemoScopeItems(prev, {
                  demo,
                  reuseExistingShowerDoor,
                  demoShowerDoorCount: demo.demoShowerDoorCount,
                  installShowerDoorCount: measurements.showerDoorCount,
                  demoOverrides,
                  showerWallTileSqft: measurements.showerWallTileSqft,
                  bathroomFloorSqft: measurements.bathroomFloorSqft,
