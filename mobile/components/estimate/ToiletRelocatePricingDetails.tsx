...
export default function ToiletRelocatePricingDetails({
  floorType,
  darkMode,
  captionColor,
  textColor,
  ...
  const details = buildToiletRelocatePricingDetails(floorType);
    ...
    <View style={{ marginTop: 8 }}>
      ...
      {expanded ? (
        <View style={[styles.panel, { borderColor, backgroundColor: panelBg }]}>
          <Text style={[styles.summary, { color: captionColor }]}>{details.detailsCopy}</Text>
          <Text style={[styles.excludesCompact, { color: captionColor }]}>{details.excludesCompact}</Text>
          ...
          {fullScopeOpen ? (
            <View style={styles.fullScopeBody}>
              ...
              <Text style={[styles.sectionHeading, { color: textColor, marginTop: 8 }]}>
                Condition assumptions
              </Text>
              {details.conditionAssumptions.map((line) => (
                <Text key={line} style={[styles.bullet, { color: captionColor }]}>
                  • {line}
                </Text>
              ))}
