...
export default function InteriorPaintPricingDetails({
  sqft,
  mobilization,
  surface,
  condition,
  ...
  const { minimumApplied } = computeInteriorPaintSuggestedTotal({
    sqft,
    mobilization,
    surface,
    condition,
  ...
  return (
    <View style={{ marginTop: 8 }}>
      ...
      {details.planningAssumption ? (
        <Text style={{ color: captionColor, fontSize: 12, fontWeight: '500', marginTop: 4, lineHeight: 16 }}>
          {details.planningAssumption}
        </Text>
      ) : null}
      {details.shortExcludesLine ? (
        <Text style={{ color: captionColor, fontSize: 11, fontWeight: '500', marginTop: 6, lineHeight: 16 }}>
          {details.shortExcludesLine}
        </Text>
      ) : null}
      ...
      {expanded ? (
        <View style={[styles.panel, { borderColor, backgroundColor: panelBg }]}>
          <Text style={[styles.disclaimer, { color: captionColor }]}>{details.surfaceBasisNote}</Text>
