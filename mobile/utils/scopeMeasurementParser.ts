...
const SQUARES_RE = /(\d[\d,]*(?:\.\d+)?)[\s-]*(?:roofing\s+)?squares?\b/gi;
const ROOF_PITCH_RE =
...
export type ParsedInsulationAssembly = {
  location: 'exterior_wall' | 'attic_ceiling' | 'floor' | null;
  materialType: string;
  rValue: string;
  sqft: number;
...
export function parseInsulationAssembliesFromNotes(
  ...
  const add = (
    ...
    assemblies.push({
      ...
      rValue: parsedRValue,
      sqft,
      ...(battFacing(clause) ? { battFacing: battFacing(clause) } : {}),
    });
  };
  for (const clause of clauses) {
    const assemblyCountBeforeClause = assemblies.length;
    ...
    if (
      hasStructuralContext &&
      assemblies.length === assemblyCountBeforeClause &&
      /\bR[-\s]?\d{2,3}\b[^.;,\n]{0,24}\binsulat(?:e|ion|ed)\b/i.test(clause)
    ) {
      assemblies.push({
        location: null,
        materialType: material(clause),
        rValue: rValue(clause),
        sqft: quantity(clause),
        ...(battFacing(clause) ? { battFacing: battFacing(clause) } : {}),
      });
    }
  }
...
export function parseScopeMeasurementsFromNotes(
  ...
  if (templateKey === 'painting' || projectType === 'painting') {
    ...
    const paintSurfaceMentioned = (surfacePattern: RegExp): boolean =>
      new RegExp(
        `(?:paint(?:ing)?|repaint(?:ing)?)\\b[^.;\\n]{0,80}${surfacePattern.source}|${surfacePattern.source}[^.;\\n]{0,80}\\b(?:paint(?:ing)?|repaint(?:ing)?)`,
        'i'
      ).test(text);
    if (paintSurfaceMentioned(/\bwall(?:s)?\b/i)) scope.push('walls');
    if (paintSurfaceMentioned(/\bceilings?\b/i)) scope.push('ceilings');
    if (
  ...
  const backsplash = (() => {
    for (const clause of clauses) {
      for (const segment of clause.split(/[,;]\s*/)) {
        ...
        const raw =
          before &&
          !/\b(?:counter|counters|countertop|quartz|granite)\b/i.test(before[0])
            ? before[1]
            : after?.[1];
  ...
  const countertopLf = pickLfNearPattern(text, /\bcountertops?|\bcounters\b/i);
  ...
  const combinedPaintLanguage =
    ...
    (out.paintScope?.includes('walls') &&
      out.paintScope?.includes('ceilings') &&
      /\b(?:paint(?:ing)?|repaint(?:ing)?)\b/i.test(text));
  const paintSqft = (() => {
    ...
    if (largestRelevantPaintSqft <= 0 && out.paintScope?.includes('walls')) {
      largestRelevantPaintSqft =
        pickSqftNearPattern(
          text,
  ...
  const exteriorWallInsulationSqft =
    ...
    insulationSqft([
      ...
      /\binsulation\b[^.;]{0,35}\bwalls?\b/i,
      /\bwalls?\s+insulation\b/i,
    ]);
  ...
  const insulationRValue = text.match(/\bR[-\s]?(\d{2,3})\b/i)?.[0];
  ...
  const airSealingExcluded = new RegExp(
  ...
  if (flooringSqft && /\bkitchen\b/i.test(blob) && /\blvp\b/i.test(blob)) {
  ...
  const wallDemoSqft = (() => {
    const quantity = String.raw`(\d[\d,]*(?:\.\d+)?)\s*(?:sq\.?\s*ft|sqft|sf|square\s+(?:foot|feet))`;
  ...
  const floorAreaSqft = (() => {
    ...
    const allowsHomeFloorInference =
      ...
      /\b(?:insulat(?:e|ion|ed)|fiberglass\s+batt|blown[-\s]?in)\b/i.test(text);
  ...
  if (
    ...
    if (!out.roofAreaSqft) {
      ...
      const sqft =
        roofTieInOnly || !roofClause ? null : firstQty(roofClause, SQFT_RE);
